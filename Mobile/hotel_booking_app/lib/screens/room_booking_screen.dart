...
class RoomBookingScreen extends StatefulWidget {
  ...
  const RoomBookingScreen({Key? key, required this.roomId, required this.maxOccupancy}) : super(key: key);
...
class _RoomBookingScreenState extends State<RoomBookingScreen> {
  ...
  Future<void> _checkAvailabilityAndPrice() async {
    if (_checkIn == null || _checkOut == null || _guests < 1) {
      setState(() { _error = 'Unesite sve podatke.'; });
      return;
    }
    if (_guests > widget.maxOccupancy) {
      setState(() { _error = 'Broj gostiju ne smije biti veći od ${widget.maxOccupancy}.'; });
      return;
    }
    setState(() { _loading = true; _error = null; _available = null; _price = null; });
    ...
    try {
      final available = await RoomsService().checkAvailability(widget.roomId, checkInStr, checkOutStr, services: servicesStr);
      if (!available) {
        setState(() { _available = false; _loading = false; _error = 'Soba nije dostupna u tom terminu.'; });
        return;
      }
      final price = await RoomsService().calculatePrice(widget.roomId, checkInStr, checkOutStr, _guests, services: servicesStr);
      print(price); 
      setState(() { _available = true; _price = price; _loading = false; });
    } catch (e) {
      print(e);
      setState(() { _error = 'Greška pri provjeri dostupnosti ili cijene.'; _loading = false; });
    }
  }
  ...
  void _goToPayment() async {
    ...
    .toList();
        ...
        final bookingId = await ReservationsService().createBooking(bookingData);
        Navigator.pop(context); // zatvori loading
        if (bookingId == 0) {
          setState(() { _error = 'Greška pri kreiranju rezervacije.'; });
          return;
        }
        Navigator.push(
          context,
        ...
        child: LayoutBuilder(builder: (context, constraints) {
          ...
          children: [
            ...
            Row(
              children: [
                Expanded(
                  child: Text(_checkIn == null ? 'Odaberite datum' : DateFormat('dd.MM.yyyy').format(_checkIn!)),
                ),
                IconButton(
            ...
            Row(
              children: [
                Expanded(
                  child: Text(_checkOut == null ? 'Odaberite datum' : DateFormat('dd.MM.yyyy').format(_checkOut!)),
                ),
                IconButton(
                  icon: const Icon(Icons.calendar_today),
            ...
            Row(
              children: [
                ...
                IconButton(
                  ...
                  onPressed: _guests < widget.maxOccupancy ? () => setState(() => _guests++) : null,
                ),
              ],
            ),
